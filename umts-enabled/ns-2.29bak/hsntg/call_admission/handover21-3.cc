...
#include "handover21-3.h"
#include "simulator.h"
#include "mip6.h"
//NC
#include "mac-802_11.h"
static class Handover3Class : public TclClass {
public:
  Handover3Class() : TclClass("Handover/Handover3") {}
  TclObject* create(int, const char*const*) {
    return (new Handover3());
  }
 ...
 */
Handover3::Handover3() : Handover(), connectingMac_(0)
{
  int i;
  for (i=0; i<10; i++)
    {
...
void Handover3::register_mih ()
{
  ...
  assert (ifMngmt_);
  debug ("At %f in %s Handover3 registers the MACs\n", NOW, \
  ...
  for (int i=0; i < (int) macs.size() ; i++) {
    ...
    //case 1: we don't want link going down and link down
    //case 2: we want link down
    //case 3: we want link going down
    switch (case_) {
    case 1:
      break;
    case 2:
    ...
    case 3:
      ...
      break;
    default:
      printf ("Error unknown case scenario\n");
      exit (-1);
    }
   }
}
 ...
 */
void Handover3::process_link_detected (link_detected_t *e)
{
  MIHAgent *mih;
  ...
  debug ("At %f in %s Handover3 link detected \n\ttype %d, bandwidth = %f, MacAddr=%d, PoA=%d\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()), \
	 config->type, config->bandwidth, e->eventSource->addr(),e->macNewPoA);
  
  //NC: do nothing if the BS belongs to the black list
  /*
  if(config->type == LINK_802_11)
    {
      if(blacklist_[e->macNewPoA] ==  1){
	goto done;
      }
    }
  */
  // Delete the detection timer
   if (detectionTimer_->status()!=TIMER_IDLE) {	
    detectionTimer_->cancel();}
   //end NC
  ...
  if (nb_info>0 )
    debug ("At %f The new interface is better...connect\n", NOW);
  else
    debug ("At %f This is the first interface...connect\n", NOW);
 ...
 * @param e The event information
 */
void Handover3::process_link_up (link_up_t *e)
{
  ...
  config = Handover::get_mac_info (e->eventSource);
  debug ("At %f in %s Handover3 received link up \n\ttype %d, bandwidth = %f, MacAddr=%d\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
 ...
 * @param e The event information
 */
void Handover3::process_link_down (link_down_t *e){
  link_parameter_config_t *config;
  
  config = Handover::get_mac_info (e->eventSource);
  //In case of 802.11 and if it was an explicit disconnect, black list the BS
  printf(" type %d, reason %d, bss_id %d \n", config->type, e->reason, ((Mac802_11*)(e->eventSource))->bss_id());
  if(config->type == LINK_802_11){
    //start scan request
    scan_request_t *req = (scan_request_t*) malloc (sizeof(scan_request_t));
    req->scanType = SCAN_TYPE_ACTIVE;
    req->action   = STOP_FIRST;
    req->nb_channels = 10;
    req->pref_ch = (int*) malloc (11*sizeof (int));
    for (int i=0 ; i <11 ; i++) {
      req->pref_ch[i] = i+1;
    }
    req->minChannelTime = 0.010;
    req->maxChannelTime = 0.020;
    req->probeDelay = 0.002;
    
    ifMngmt_->get_mih()->mih_scan (ifMngmt_, e->eventSource, req);
    
    //if (e->reason ==  RC_EXPLICIT_DISCONNECT){
    //  blacklist_[((Mac802_11*)(e->eventSource))->bss_id()] = 1;
    //}  
  }
  detectionTimer_->attach_event(e);
  printf ("timeout=%f",detectionTimeout_);
  detectionTimer_->resched (detectionTimeout_);
}
 ...
 * @param e The event information
 */
void Handover3::process_scan_response (mih_scan_response_t *e)
{
  //to be defined by subclass
  scan_response_t *resp = (scan_response_t*) e->result;
  BSSDescription *n;
  n=resp->bssDesc_head.lh_first;
  if (!n) {
    //we did not find any AP
    printf ("No AP found\n");
    if (detectionTimer_->status()==TIMER_PENDING)
      detectionTimer_->resched (0);
    else
      debug ("Timer already expired\n");
    return;
  }
  for ( ; n ; n=n->next_entry()) {
    //create a link detected to process
    link_detected_t *e2 = (link_detected_t*) malloc (sizeof (link_detected_t));
    e2->eventSource = e->eventSource;
    e2->macMobileTerminal = e->eventSource->addr();
    e2->macNewPoA = n->bss_id();
    e2->mihCapability = 1; 
    process_link_detected (e2);
  }
}

/*
 * If no wlan detected, handover to UMTS
 * @param e The event information
 *
 */
void Handover3::process_no_link_detected(void *e2){
  link_down_t *e = (link_down_t *) e2;
  ...
  debug ("At %f in %s Handover3 received link down \n\tMacAddr=%d\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
	 e->eventSource->addr());
 ...
 * @param e The event information
 */
void Handover3::process_link_going_down (link_going_down_t *e)
{
  ...
  debug ("At %f in %s Handover3 received Link going down \n\t probability = %d%%\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
	 e->confidenceLevel);

  if (e->confidenceLevel > 80 ) {
    MIHAgent *mih;
 ...
 */
void Handover3::process_exp_prefix (exp_prefix* data)
{
  Mac *mac;
  Tcl& tcl = Tcl::instance();

  debug ("At %f in %s Handover3 received expired prefix %s\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
 ...
 */
void Handover3::process_new_prefix (new_prefix* data)
{
  MIHAgent *mih;
  link_parameter_config_t *config;
  link_parameter_config_t *config2;
  ...
  debug ("At %f in %s Handover3 received new prefix %s\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
	 Address::instance().print_nodeaddr(data->prefix));
  ...
  nb_mac_infos = ifMngmt_->get_mih ()->get_nb_interfaces ();
  //for each interface
  printf ("NB flows=%d, NB mac=%d", (int)flows.size(), nb_mac_infos);
...
int Handover3::compute_new_address (int prefix, Node *interface)
{
  int new_address;
