...
#include "handover-qos.h"
#include "simulator.h"
#include "mip6.h"

static class HandoverQosClass : public TclClass {
public:
  HandoverQosClass() : TclClass("Handover/QoS") {}
  TclObject* create(int, const char*const*) {
    return (new HandoverQos());
  }
} class_handoverqos;
 ...
 */
HandoverQos::HandoverQos() : Handover(), connectingMac_(0)
{
  ptd_ = 0;
  pdv_ = 0;
  plr_ = 0;
  ...
  bind ("plr_", &plr_);
  bind ("per_", &per_);
  bind ("throughput_", &throughput_);
  bind ("app_start_time_", &app_start_);
  if (app_start_ != -1)
    ctimer_ = new ConfTimer (this, app_start_);
  else
    printf ("No application registered\n");
}

void HandoverQos::register_mih ()
{
  MIHAgent *mih;
  vector <Mac *> macs;

  assert (ifMngmt_);
  debug ("At %f in %s HandoverQos registers the MACs\n", NOW, \
  ...
  for (int i=0; i < (int) macs.size() ; i++) {
    ...
    mih->event_register (ifMngmt_, macs.at (i), MIH_LINK_PARAMETERS_CHANGE);
  }
}
 ...
 */
void HandoverQos::set_link_thresholds () 
{
  MIHAgent *mih;
  vector <Mac *> macs;
  ...
  mih = ifMngmt_->get_mih();
  macs = mih->get_local_mac ();
  printf ("Handover setting threshold\n");

  for (int i=0; i < (int) macs.size() ; i++) {
    struct link_param_th thresholdList;
    //thresholdList.parameter= LINK_RX_DATA_THROUGHPUT;
    thresholdList.parameter=LINK_PACKET_DELAY;
    thresholdList.initActionTh.data_d = 0.006;
    thresholdList.rollbackActionTh.data_d = 0.004;
    thresholdList.exectActionTh.data_d = 0.007;
    mih->mih_configure_thresholds (macs.at (i), 1, &thresholdList);
  }  
}
 ...
 * @param e The event information
 */
void HandoverQos::process_link_detected (link_detected_t *e)
{
  MIHAgent *mih;
  ...
  debug ("At %f in %s HandoverQos link detected \n\ttype %d, bandwidth = %f, MacAddr=%d, PoA=%d\n", \
 ...
 * @param e The event information
 */
void HandoverQos::process_link_up (link_up_t *e)
{
  ...
  config = Handover::get_mac_info (e->eventSource);
  debug ("At %f in %s HandoverQos received link up \n\ttype %d, bandwidth = %f, MacAddr=%d\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
	 config->type, config->bandwidth, e->eventSource->addr());

  if (config->type == LINK_802_11 || config->type == LINK_802_16) {
    //we need to check prefix first.
    ((MIPV6Agent*)ifMngmt_)->send_rs (e->eventSource);
  }
 ...
 * @param e The event information
 */
void HandoverQos::process_link_down (link_down_t *e)
{
  ...
  vector <Agent *> agents; //agent to redirect


  assert (ifMngmt_);

  debug ("At %f in %s HandoverQos received link down \n\tMacAddr=%d\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
	 e->eventSource->addr());
  ...
  if (found) {
	...
	agents.push_back (flows.at(i)->remote());
	flows.at(i)->update_interface(new_node);
      }
    }
    if (agents.size()>0)
      ((MIPV6Agent*) ifMngmt_)->send_update_msg (agents,new_node);
  }
  else {
 ...
 * @param e The event information
 */
void HandoverQos::process_link_going_down (link_going_down_t *e)
{
  ...
  debug ("Link going down...we disconnect it\n");
    //We look if there is any flow that uses this interface.
  //if yes, then we need to switch to another interface
  MIHAgent *mih;
  link_parameter_config_t *config;
  ...
  for (int mac_index = best_index+1 ; mac_index < nb_mac_infos ; mac_index++) {
	...
	&& mac_infos[mac_index]->status_ == LINK_STATUS_UP) {
      ...
      config2 = Handover::get_mac_info (mac_infos[mac_index]->mac_);
      if ( (best_type == LINK_802_11 && config2->type == LINK_ETHERNET)
	   || (best_type == LINK_802_16 && (config2->type == LINK_ETHERNET||config2->type == LINK_802_11))
	   || (best_type == LINK_UMTS && (config2->type == LINK_802_11 || config2->type == LINK_802_16
					  || config2->type == LINK_ETHERNET))){
	best_type = config2->type;
	best_index = mac_index;
      }
  ...
  if (found) {
    ...
    //we transfert the flows 
    for (int i=0 ; i < (int)flows.size() ; i++) {
      char *str1 = Address::instance().print_nodeaddr(flows.at(i)->interface()->address());
	...
	char *str1 = Address::instance().print_nodeaddr(new_node->address());
	debug ("Must redirect this flow to use interface %s\n",str1);
	delete []str1;
	agents.push_back (flows.at(i)->remote());
	tcl.evalf ("%s target [%s entry]", flows.at(i)->local()->name(), new_node->name());
    ...
    if (agents.size()>0)
      ((MIPV6Agent*) ifMngmt_)->send_update_msg (agents,new_node);
    //we disconnect this one
    //mih->link_disconnect (e->eventSource);
  }
  else {
    //TMP
    for (int mac_index = 0 ; mac_index < nb_mac_infos ; mac_index++) {
      config2 = Handover::get_mac_info (mac_infos[mac_index]->mac_);
      if (config2->type == LINK_802_16) {
	Node *new_node;
	Tcl& tcl = Tcl::instance();
	tcl.evalf ("%s get-node", mac_infos[mac_index]->mac_->name());
	new_node = (Node*) TclObject::lookup(tcl.result());
	tcl.evalf ("%s target [%s entry]", flows.at(0)->local()->name(), new_node->name());
	flows.at(0)->update_interface(new_node);
	mac_infos[mac_index]->mac_->link_connect(0);
	break;
      }
    }
 ...
 * @param e The event information
 */
void HandoverQos::process_link_parameter_change (link_parameter_change_t *e)
{
  debug ("At %f in %s HandoverQos received link parameters change \n\tMacAddr=%d\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
	 e->eventSource->addr());

  for (int i = 0 ; i < e->numLinkParameters; i++) {
    printf ("Parameter: %d, old_value=%f, new_value=%f\n",e->linkParameterList[i].parameter, e->linkParameterList[i].old_value.data_d, e->linkParameterList[i].new_value.data_d);
    
    link_going_down_t *ne = (link_going_down_t*) malloc (sizeof (link_going_down_t));
    ne->eventSource = e->eventSource;
    process_link_going_down (ne);
 ...
 */
void HandoverQos::process_exp_prefix (exp_prefix* data)
{
  Mac *mac;
  Tcl& tcl = Tcl::instance();

  debug ("At %f in %s HandoverQos received expired prefix %s\n", \
	 NOW, Address::instance().print_nodeaddr(ifMngmt_->addr()),\
 ...
 */
void HandoverQos::process_new_prefix (new_prefix* data)
{
  MIHAgent *mih;
  link_parameter_config_t *config;
  link_parameter_config_t *config2;
  ...
  char *str1 = Address::instance().print_nodeaddr(ifMngmt_->addr());
  char *str2 = Address::instance().print_nodeaddr(data->prefix);
  debug ("At %f in %s HandoverQos received new prefix %s\n", NOW, str1, str2);
  delete []str1;
  delete []str2;
  ...
  //for each interface
  for (int mac_index = 0 ; mac_index < nb_mac_infos ; mac_index++) {
    //if (mac_infos[mac_index]->mac_ != mac) { //since it is new prefix...
      //this is not the new interface
      //get mac information
      config2 =  Handover::get_mac_info (mac_infos[mac_index]->mac_);
      //compare to the new one
      if (mac_infos[mac_index]->mac_ == mac || config2->type == LINK_UMTS && (config->type == LINK_802_11 || config->type == LINK_802_16 || config->type == LINK_ETHERNET)
	  || (config2->type == LINK_802_16 && (config->type == LINK_802_11 || config->type == LINK_ETHERNET))
	  || (config2->type == LINK_802_11 && config->type == LINK_ETHERNET)) {
	...
	for (int i=0 ; i < (int)flows.size() ; i++) {
	  ...
	  if ( flows.at(i)->interface() == node){
	    //we redirect this flow to the new interface
	    char *str1 = Address::instance().print_nodeaddr(flows.at(i)->interface()->address());
	    char *str2 = Address::instance().print_nodeaddr(new_node->address());
...
int HandoverQos::compute_new_address (int prefix, Node *interface)
{
  int new_address;
