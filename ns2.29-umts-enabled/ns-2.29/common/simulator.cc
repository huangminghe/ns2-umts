 ...
 * $Id: simulator.cc,v 1.1.1.1 2006/03/08 13:52:49 rouil Exp $
 *
...
void Simulator::populate_hier_classifiers() {
	...
	for (int i=0; i<nn_; i++) {
		if (nodelist_[i] == NULL) {
			//NIST: why skipping 2 items..comment i++
			//i++; 
			//end NIST
			continue;
		}
...
int Simulator::node_id_by_addr(int address) {
	for (int i=0; i<nn_; i++) {
		//NIST: bug fixing, there are some cases where nodelist_[i] is null 
		//if(nodelist_[i]->address() == address) //old code
		if((nodelist_[i]!=NULL)&&(nodelist_[i]->address() == address)) //new code
		//end NIST
